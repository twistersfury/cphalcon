...
PHP_METHOD(Phalcon_Cache_AbstractCache, doGetMultiple)
{
	...
	zend_long ZEPHIR_LAST_CALL_STATUS;
	zval *keys, keys_sub, *defaultValue = NULL, defaultValue_sub, __$null, adapterClass, element, results, serializer, _0, _1$$3, _2$$3, _3$$3, _4$$3, _5$$3, *_6$$5, _7$$5, _8$$6, _10$$7;
	zval *this_ptr = getThis();
	...
	if (ZEPHIR_IS_STRING_IDENTICAL(&adapterClass, "Phalcon\Cache\Adapter\Redis")) {
		...
		ZEPHIR_INIT_NVAR(&_4$$3);
			ZEPHIR_INIT_NVAR(&_4$$3);
			object_init_ex(&_4$$3, phalcon_0__closure_ce);
			if (zephir_has_constructor(&_4$$3)) {
				ZEPHIR_CALL_METHOD(NULL, &_4$$3, "__construct", NULL, 0, &serializer, defaultValue);
				zephir_check_call_status();
			}

		zephir_update_property_zval(_4$$3, ZEND_STRL("serializer"), &serializer);
		zephir_update_property_zval(_4$$3, ZEND_STRL("defaultValue"), defaultValue);
		...
		if (Z_TYPE_P(keys) == IS_ARRAY) {
			ZEND_HASH_FOREACH_VAL(Z_ARRVAL_P(keys), _6$$5)
			{
				ZEPHIR_INIT_NVAR(&element);
				ZVAL_COPY(&element, _6$$5);
				ZEPHIR_CALL_METHOD(&_8$$6, this_ptr, "get", &_9, 0, &element, defaultValue);
				zephir_check_call_status();
				zephir_array_update_zval(&results, &element, &_8$$6, PH_COPY | PH_SEPARATE);
			} ZEND_HASH_FOREACH_END();
		} else {
			ZEPHIR_CALL_METHOD(NULL, keys, "rewind", NULL, 0);
			zephir_check_call_status();
			while (1) {
				ZEPHIR_CALL_METHOD(&_7$$5, keys, "valid", NULL, 0);
				zephir_check_call_status();
				...
				zephir_check_call_status();
					ZEPHIR_CALL_METHOD(&_10$$7, this_ptr, "get", &_11, 0, &element, defaultValue);
					zephir_check_call_status();
					zephir_array_update_zval(&results, &element, &_10$$7, PH_COPY | PH_SEPARATE);
				ZEPHIR_CALL_METHOD(NULL, keys, "next", NULL, 0);
				zephir_check_call_status();
			}
		}
